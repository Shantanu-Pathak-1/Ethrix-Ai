...
async def set_plan(request: Request, email: str = Form(...), plan: str = Form(...)):
    ...
    # plan field set karo + is_pro backward compat
    await db_module.users_collection.update_one(
        {"email": email},
        {"$set": {
            "plan":   plan,
            "is_pro": plan in ("pro", "elite"),
            "daily_usage": {}  # usage reset on plan change
        }}
    )
...
# Legacy routes — purane promote/demote buttons bhi kaam karte rahein
